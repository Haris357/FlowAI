...
export function generateQuotePDF(quote: QuoteData, company: CompanyData): Buffer {
  ...
  // ── Customer Info + Quote Meta (two columns) ──
  const metaStartY = y;
  ...
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(...palette.brand);
  doc.text('QUOTE TO', ML, y);
  y += 5;

  doc.setFontSize(11);
  doc.setFont('helvetica', 'bold');
  doc.setTextColor(...palette.text);
  doc.text(quote.customerName || '-', ML, y);
  y += 4.5;
  ...
  if (quote.customerEmail) {
    ...
    doc.text(quote.customerEmail, ML, y);
    y += 4;
  }
  ...
  const metaRows: [string, string, boolean][] = [
    ['Quote #', quote.quoteNumber || '-', false],
    ['Issue Date', formatDate(quote.issueDate), false],
    ['Expiry Date', formatDate(quote.expiryDate), false],
    ['Status', (quote.status || 'draft').toUpperCase(), true],
  ];
  ...
  drawRow('Subtotal', fmt(quote.subtotal));

  if (quote.taxRate > 0 || quote.taxAmount > 0) {
    const taxLabel = quote.taxRate > 0 ? `Tax (${quote.taxRate}%)` : 'Tax';
    drawRow(taxLabel, fmt(quote.taxAmount));
  }

  if (quote.discount > 0) {
    drawRow('Discount', `- ${fmt(quote.discount)}`);
  }
  ...
  if (quote.notes) {
    ...
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(...palette.textMid);
    const noteLines = doc.splitTextToSize(quote.notes, CONTENT_W);
  ...
  if (quote.terms) {
    ...
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(...palette.textMid);
    const termLines = doc.splitTextToSize(quote.terms, CONTENT_W);
