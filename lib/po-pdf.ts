...
export function generatePurchaseOrderPDF(po: PurchaseOrderData, company: CompanyData): Buffer {
  ...
  doc.text('PURCHASE ORDER', PW - MR, y, { align: 'right' });
  y += 6;
  ...
  // ── Vendor Info + PO Meta (two columns) ──
  const metaStartY = y;
  ...
  const metaRows: [string, string, boolean][] = [
    ['PO #', po.poNumber || '-', false],
    ['Issue Date', formatDate(po.issueDate), false],
    ...(po.expectedDate ? [['Expected Delivery', formatDate(po.expectedDate), false] as [string, string, boolean]] : []),
    ['Status', (po.status || 'draft').toUpperCase(), true],
  ];
  ...
  // ── Line Items Table ──
  const tableBody = (po.items || []).map(item => [
    item.description || '',
  ...
  drawRow('Subtotal', fmt(po.subtotal));

  if (po.taxRate > 0 || po.taxAmount > 0) {
    const taxLabel = po.taxRate > 0 ? `Tax (${po.taxRate}%)` : 'Tax';
    drawRow(taxLabel, fmt(po.taxAmount));
  }

  if (po.discount > 0) {
    drawRow('Discount', `- ${fmt(po.discount)}`);
  }
  ...
  if (po.notes) {
    ...
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(...palette.textMid);
    const noteLines = doc.splitTextToSize(po.notes, CONTENT_W);
  ...
  if (po.terms) {
    ...
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(...palette.textMid);
    const termLines = doc.splitTextToSize(po.terms, CONTENT_W);
