...
function SkeletonCard() {
  return (
    <div className="rounded-2xl liquid-glass overflow-hidden animate-pulse">
      <div className="h-48 bg-slate-200/50 dark:bg-white/[0.04]" />
      <div className="p-5 space-y-3">
        <div className="h-4 w-20 bg-slate-200/50 dark:bg-white/[0.06] rounded" />
        <div className="h-5 w-3/4 bg-slate-200/50 dark:bg-white/[0.06] rounded" />
        <div className="h-4 w-full bg-slate-200/50 dark:bg-white/[0.06] rounded" />
        <div className="h-4 w-2/3 bg-slate-200/50 dark:bg-white/[0.06] rounded" />
        <div className="flex items-center gap-3 pt-2">
          <div className="w-8 h-8 rounded-full bg-slate-200/50 dark:bg-white/[0.06]" />
          <div className="h-3 w-24 bg-slate-200/50 dark:bg-white/[0.06] rounded" />
          <div className="h-3 w-16 bg-slate-200/50 dark:bg-white/[0.06] rounded ml-auto" />
        </div>
      </div>
    </div>
  );
...
export default function BlogPage() {
  ...
  return (
    <div className="min-h-screen w-full bg-white dark:bg-[#1A1915] font-sans text-slate-900 dark:text-[#EEECE8] selection:bg-brand-100 selection:text-brand-900 overflow-x-hidden text-[15px]">
      ...
      {/* ============ HERO ============ */}
      <section className="relative pt-32 pb-12 lg:pt-36 lg:pb-16">
        <div
              ...
              <button
                ...
                className={`px-4 py-1.5 rounded-full text-[13px] font-medium transition-all ${
                  activeCategory === cat
                    ...
                    : 'liquid-glass-subtle text-slate-600 dark:text-[#A8A29E] hover:bg-slate-200/60 dark:hover:bg-white/[0.1]'
                }`}
              >
                {cat}
              </button>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredPosts.map((post) => (
                <Link
                  key={post.id}
                  href={`/blog/${post.slug}`}
                  className="group rounded-2xl liquid-glass overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300"
                >
